...
public class BillController {
        ...
        try {
            ...
            if (validateResquestMap(requestMap)) {
                if (requestMap.containsKey("isGenerate") && !(Boolean) requestMap.get("isGenerate")) {
                    ...
                    filename = InventoryUtils.getUUID();
                    requestMap.put("uuid", filename);
                    insertBill(requestMap);
                }

                // print user data (name , email m contactNumber , ...)
                String data = "Name: " + requestMap.get("username") + "\n"
                        + "Contact Number: " + requestMap.get("contactNumber") + "\n"
                        + "Address: " + requestMap.get("address") + "\n"
                        + "Payment Method: " + requestMap.get("paymentMethod") + "\n"
                        + "Date: " + LocalDateTime.now().toString();
                Document document = new Document();
                PdfWriter.getInstance(document, new FileOutputStream(InventoryConstants.STORE_LOCATION + "\\" + filename + ".pdf"));
                document.open();
                ...
                // Print table data
                JSONArray jsonArray = InventoryUtils.getJsonArrayFromString((String) requestMap.get("productDetails"));
                for (int i = 0; i < jsonArray.length(); i++) {
                    addRows(table, InventoryUtils.getMapFromJson(jsonArray.getString(i)));
                }
            ...
            return InventoryUtils.getResponseEntity("Required data not found", HttpStatus.BAD_REQUEST);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    public ResponseEntity<List<Bill>> getBills() {
        ...
        if (jwtRequestFilter.isAdmin()) {
            ...
            list = billRepo.getBillByUserName(jwtRequestFilter.getCurrentUsername());
        }
    ...
    public ResponseEntity<byte[]> getPdf(@RequestBody Map<String, Object> requestMap) {
        ...
        try {
            ...
            String filepath = InventoryConstants.STORE_LOCATION + "\\" + (String) requestMap.get("uuid") + ".pdf";

            if (InventoryUtils.isFileExist(filepath)) {
        ...
        try {
            if (jwtRequestFilter.isAdmin()) {
                ...
                if (!optional.isEmpty()) {
                    ...
                    return InventoryUtils.getResponseEntity("Bill is deleted successfully", HttpStatus.OK);
                }
                //System.out.println("Product id doesn't exist");
                return InventoryUtils.getResponseEntity("Bill id doesn't exist", HttpStatus.OK);
            } else {
    ...
    private void insertBill(Map<String, Object> requestMap) {
        try {
            ...
            bill.setName((String) requestMap.get("username"));
